...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--data_dir", type=str, default="./Dataset", help="data directory (target + pretrain)")

    p.add_argument("--dataset", type=str, default="SAMYANG_dataset", help="dataset name without .csv")
    p.add_argument("--target", required=True, type=str)
    p.add_argument("--minute_interval", type=int, default=15)

    p.add_argument("--ft_target_only", action="store_true",
                help="Fine-tuning loss uses target channel only (ignore covariates in loss).")
    ...
    p.add_argument("--pretrain_files", nargs="+",
                default=["ai4i2020.csv", "IoT.csv", "Steel_industry.csv"],
                help="pretraining dataset files (space-separated)")
    ...
    p.add_argument("--ft_batch_size", type=int, default=None)
    p.add_argument("--ft_learning_lr", type=float, default=None)
    ...
    p.add_argument("--pt_epochs", type=int, default=None)
    p.add_argument("--pt_batch_size", type=int, default=None)
    p.add_argument("--pt_learning_lr", type=float, default=None)
...
def main():
    ...
    data_file = dataset if dataset.endswith(".csv") else f"{dataset}.csv"
    ...
    overrides = {
        ...
        "finetune_epochs": int(args.ft_epochs) if args.ft_epochs is not None else None,
        "finetune_batch_size": args.ft_batch_size,
        "finetune_lr": args.ft_learning_lr,
        "weight_decay": args.weight_decay,
        "head_dropout": args.head_dropout,

        "pretrain_epochs": args.pt_epochs,
        "pretrain_batch_size": args.pt_batch_size,
        "pretrain_lr": args.pt_learning_lr,
        "mask_ratio": args.mask_ratio,
        "pretrain_grad_clip": args.pretrain_grad_clip,
        "ft_target_only": bool(args.ft_target_only),
    ...
    if want_seq or want_soft:
        ...
        if args.pretrain_files:
            pretrain_files = list(args.pretrain_files) if args.pretrain_files else list(cfg.get("pretrain_files", []))
        else:
    ...
    try:
        if want_soft:
            cfg_soft = dict(cfg)
            results["soft_masking"] = _run_cl(cfg_soft, device, arrays, data_dir, data_file, args.target, out_root, "soft_masking", True)
            clear_memory()
        if want_seq:
            cfg_seq = dict(cfg)
            cfg_seq["use_soft_masking"] = False
            results["sequential"] = _run_cl(cfg_seq, device, arrays, data_dir, data_file, args.target, out_root, "sequential", False)
            clear_memory()
        if want_baseline:
            cfg_base = dict(cfg)
            results["baseline"] = _run_baseline(cfg_base, device, data_dir, data_file, args.target, out_root)
            clear_memory()
    finally:
